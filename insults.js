...
function randomInsult(insults) {
    let random = Object.keys(insults);

    return random[Math.floor(Math.random() * random.length)];
}

insultButton.addEventListener('click', () => {
    let input = document.querySelector('.insults--input');

    input.innerHTML = insults[randomInsult(insults)].insult;
})
